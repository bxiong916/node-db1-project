  ...
  return async (req, res, next) => {
    try {
      ...
      if (!name || !budget) {
        ...
        res
          .status(400)
          .json({ message: "name of account must be in string format" });
      } else if (typeof budget !== "number") {
        res.status(400).json({ message: "budget must be in number format" });
      } else if (name.trim().length < 3 || name.trim().length > 100) {
        res.status(400).json({
          message:
            "account name must be longer than three and less than one hundred characters",
        });
      } else if (budget < 0 || budget > 1000000) {
        res.status(400).json({
          message:
            "amount within account must be greater than zero and less than one million",
        });
      } else {
